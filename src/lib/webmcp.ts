...
export function registerFlockdocWebMCP({ modelContext, actions }: { modelContext?: WebMCPModelContext; actions: FlockdocActions }) {
  ...
  const tools: WebMCPTool[] = [
    ...
    { name: 'flockdoc.create', description: 'Create a Paper, Spreadsheet, or Diagram at an optional folder path prefix.', inputSchema: objectSchema({ type: { enum: ['paper', 'spreadsheet', 'diagram'] }, name: { type: 'string' }, prefix: { type: 'string' } }, ['type', 'name']), execute: actions.createFlockdoc },
