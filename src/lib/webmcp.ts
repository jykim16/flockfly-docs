...
export function registerFlockdocWebMCP({ modelContext, actions }: { modelContext?: WebMCPModelContext; actions: FlockdocActions }) {
  ...
  const tools: WebMCPTool[] = [
    ...
    { name: 'flockdoc.create', description: 'Create a Paper, Spreadsheet, Diagram, or Presentation at an optional folder path prefix.', inputSchema: objectSchema({ type: { enum: ['paper', 'spreadsheet', 'diagram', 'presentation'] }, name: { type: 'string' }, prefix: { type: 'string' } }, ['type', 'name']), execute: actions.createFlockdoc },
    ...
    { name: 'flockdoc.move', description: 'Move a flockdoc to a folder path prefix, or use an empty prefix for the workspace root.', inputSchema: objectSchema({ id: { type: 'string' }, prefix: { type: 'string' } }, ['id', 'prefix']), execute: actions.moveFlockdoc },
    { name: 'flockdoc.delete', description: 'Move a flockdoc to recoverable trash.', inputSchema: objectSchema({ id: { type: 'string' } }, ['id']), execute: actions.deleteFlockdoc },
  ];
