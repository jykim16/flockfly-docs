...
describe('Flockdoc API client', () => {
  ...
  it('maps backend metadata and loads native snapshot state', async () => {
    ...
    expect(listed.flockdocs[0]).toMatchObject({ id: 'flockdoc_1', parentFolderId: 'folder_1', modifiedAt: '2026-08-29T00:00:00Z', headRevision: 2 });
  ...
  it('creates and lists folders, creates and moves files in folders, and soft-deletes files', async () => {
    const backendFolder = { id: 'folder_1', name: 'Planning', parentFolderId: null, updatedAt: '2026-08-30T00:00:00Z' };
    const folder = { id: 'folder_1', name: 'Planning', parentFolderId: null, modifiedAt: '2026-08-30T00:00:00Z' };
    const moved = { id: 'flockdoc_1', type: 'paper', name: 'Plan', parentFolderId: 'folder_1', updatedAt: '2026-08-30T00:00:00Z' };
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(new Response(JSON.stringify({ folders: [backendFolder] }), { status: 200 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({ folder: backendFolder }), { status: 201 }))
    ...
    await expect(api.listFolders()).resolves.toEqual({ folders: [folder] });
    await api.createFolder('Planning', null);
    await api.create('Plan', 'paper', 'folder_1');
    await api.move('flockdoc_1', 'folder_1');
    ...
    expect(fetchMock.mock.calls.map(([url, init]) => [url, init.method, init.body ? JSON.parse(init.body) : null])).toEqual([
      ['/v1/flockdoc-folders', 'GET', null],
      ['/v1/flockdoc-folders', 'POST', { name: 'Planning', parentFolderId: null }],
      ['/v1/flockdocs', 'POST', { name: 'Plan', type: 'paper', parentFolderId: 'folder_1' }],
      ['/v1/flockdocs/flockdoc_1', 'PATCH', { parentFolderId: 'folder_1' }],
