...
export function SpreadsheetEditor({ item, onBack, onRename, onSnapshot }: SpreadsheetEditorProps) {
  ...
  return <main className="editor-shell univer-shell">
    <header className="editor-header"><button aria-label="Back to workspace" onClick={onBack}><ArrowLeft /></button><div><input className="document-title" aria-label="Spreadsheet name" value={item.name} onChange={event => onRename(event.target.value)} /><span>{status}</span></div><button className="share"><Share2 /> Share</button><span className="avatar">You</span></header>
